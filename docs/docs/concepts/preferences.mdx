...
Your desired catalog lives in your code; the real one lives in Bodhveda. They drift the moment you ship a new event and forget to update Bodhveda to match — and because email defaults to off, that drift is **silent**: the send returns `200`, the in-app half keeps working, and the email simply never arrives.
...
<Warning>
    Run it on **every** deploy, not just the deploy that adds an event. A catalog seeded by hand is a catalog that silently falls behind the code that depends on it.
</Warning>

To diagnose a missing email after the fact, read the send back with [retrieve a notification](/api-reference/endpoint/notifications/get-notification) — `email.failure_reason` will be `not_cataloged`.
