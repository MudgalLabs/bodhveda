...
type Preference struct {
	...
	Event   string
	Enabled bool
	// Only used for project preferences, not recipient preferences.
	Label     string
	CreatedAt time.Time
	UpdatedAt time.Time
}

func NewProjectPreference(projectID *int, recipientID *int, channel string, topic string, event string, label string, enabled bool) *Preference {
	now := time.Now().UTC()
